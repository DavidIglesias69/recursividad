...
public class Recorsividad {
	...
	public static void main(String[] args) {
		System.out.println( esBinario(10111111));

		System.out.println(numeroBinario (231));
	...
	public static String numeroBinario (int numero) {
		...
		return numeroBinario(numero/2)+numero%2; 
	...
	public static boolean esBinario (int num) {
		...
		boolean ultimoDigito = num%10==0 || num%10==1;
		return ultimoDigito && esBinario(num/10);
		
	}
public boolean estaOrdenadoCreciente (String num) {
	
	if (num.length()==1) {
		return true;
	}
	boolean ordenado=num.charAt(0) <= num.charAt(1);
	return ordenado && ordenarPalabra (num.substring(1,num.length()-1));

}
public boolean estaOrdenadoDecreciente (String num) {

	
	if (num.length()==1) {
		return true;
	}
	boolean ordenado=num.charAt(1) <= num.charAt(0);
	return ordenado && ordenarPalabra (num.substring(1,num.length()-1));
